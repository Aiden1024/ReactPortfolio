  ...
  return (
    ...
    >
      <div className=" flex flex-row items-end justify-center z-20">
        <button
          className="text-5xl font-signature ml-2 hover:text-rose-100 duration-200"
        ...
        <button
          className={` ${
            localStorage.getItem("currLang") === "cn"
      ...
      <div
        onClick={() => setNav(!nav)}
        className="cursor-pointer pr-z z-10 text-gray-500 md:hidden"
      >
      ...
      {/* only if nav is true, SHOW */}
      {
        ...
        bg-gradient-to-br from-black via-gray-800 to-red-900 text-gray-300 ${
        ...
        >
          {links.map(({ id, link }) => (
            <li
              key={id}
              className="px-4 cursor-pointer capitalize py-6 text-4xl focus:text-red-800"
            >
              <Link
                to={link}
                onClick={() => setNav(!nav)}
